...
def test_valid_phase_address():
    from phaselib import is_valid_phase_address

    main = valid_phase_address()
    test = valid_phase_address('testnet')

    assert is_valid_phase_address(main) is True
    assert is_valid_phase_address(main, 'mainnet') is True
    assert is_valid_phase_address(main, 'testnet') is False

    assert is_valid_phase_address(test) is False
    assert is_valid_phase_address(test, 'mainnet') is False
    assert is_valid_phase_address(test, 'testnet') is True


def test_invalid_phase_address():
    from phaselib import is_valid_phase_address

    main = invalid_phase_address()
    test = invalid_phase_address('testnet')

    assert is_valid_phase_address(main) is False
    assert is_valid_phase_address(main, 'mainnet') is False
    assert is_valid_phase_address(main, 'testnet') is False

    assert is_valid_phase_address(test) is False
    assert is_valid_phase_address(test, 'mainnet') is False
    assert is_valid_phase_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from phaselib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % phaselib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(phaselib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(phaselib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(phaselib.blocks_to_seconds(16616)) == 2612035
